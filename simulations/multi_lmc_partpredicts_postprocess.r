...
for(i in 1:20){
  ...
  load(g("simulations/lmc_m/spiox_partpredicts_mhfull_{i}.RData"))
  load(g("simulations/lmc_m/spiox_partpredicts_gibbs_{i}.RData"))
  load(g("simulations/lmc_m/spiox_partpredicts_clust_{i}.RData"))
  load(g("simulations/lmc_m/meshed_partpredicts_{i}.RData"))
  load(g("simulations/lmc_m/spiox_nnsp_{i}.RData"))
  
  meshed_yhat <- spmeshed_out %>% with(
    coordsdata %>% dplyr::select(-forced_grid) %>% 
      cbind(data.frame(Ymeshed=yhat_mcmc %>% meshed:::summary_list_mean()))
  )
...
save(file=g("simulations/lmc_m/partpredicts_results.RData"), list=c("pp_results"))
