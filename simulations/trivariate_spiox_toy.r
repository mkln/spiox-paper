...
#for(ii in 1:3){
  ...
  Omega <- matrix(c(1,0,0.7,0,1,-.5,0.7,-.5,1), ncol=3)
  ...
  if(T){
    load(g("simulations/trivariate_spiox_toy_reps/data_{ii}.RData"))
  } else {
    D <- rep(1e-3, q)
    nulist <- c(nu1, nu2, nu3)
    
    Clist <- 1:q %>% lapply(\(j) spiox::Correlationc(cx_all, cx_all, c(phi, 1, nulist[j], D[j]), TRUE, TRUE) )
    ...
    save(file=glue::glue("simulations/trivariate_spiox_toy_reps/data_{ii}.RData"), 
         list=c("simdata", 
  ...
  if(T){
    ...
    theta_opts <- rbind(c(30, 31, 29), c(1, 0.9, 1), c(nu1, nu2, nu3), c(1e-3, 1e-4, 1e-2))
    ...
    Y_out <- Y[which_out,]
    Yhat_spiox <- spiox_predicts$Y %>% apply(1:2, mean)
    ...
    save(file=glue::glue("simulations/trivariate_spiox_toy_reps/spiox_{ii}.RData"), 
  ...
  if(T){
    ...
    save(file=glue::glue("simulations/trivariate_spiox_toy_reps/spiox_latentq_{ii}.RData"), 
  ...
  if(T){
    ...
    save(file=glue::glue("simulations/trivariate_spiox_toy_reps/spiox_latentn_{ii}.RData"), 
  ...
  if(T){
    ...
    meshed_time <- system.time({
      spmeshed_out <- meshed::spmeshed(y=Y_meshed, x=X, coords=cx_all, k=q, family = "gaussian",
                                       block_size=30, n_samples = keep, n_thin = 1, n_burn = burn, 
                                       n_threads = n_threads, verbose = 10,
    ...
    save(file=glue::glue("simulations/trivariate_spiox_toy_reps/meshed_{ii}.RData"), 
  ...
  if(T){
    ...
    save(list=c("fit4", "gpgpm"),file = glue::glue("simulations/trivariate_spiox_toy_reps/gpgpm_{ii}.RData"))
