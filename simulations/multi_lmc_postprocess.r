...
for(i in 1:20){
  ...
  load(g("simulations/lmc_m/data_{i}.RData"))
  ...
  load(g("simulations/lmc_m/spiox_nnsp_{i}.RData"))
  time_nnsp <- total_time
  load(g("simulations/lmc_m/spiox_mhfull_{i}.RData"))
  time_metrop <- total_time
  load(g("simulations/lmc_m/spiox_gibbs_{i}.RData"))
  time_gibbs <- total_time
  load(g("simulations/lmc_m/spiox_clust_{i}.RData"))
  time_clust <- total_time
  load(g("simulations/lmc_m/meshed_{i}.RData"))
  ...
  # meshed_time
  load(g("simulations/lmc_m/nngp_{i}.RData"))
  nngp_predicts <- list()
  ...
  for(j in 1:24){
    ...
    nngp_phi[j] <- nngp_results[[j]]$fitmodel$p.theta.samples[,"phi"] %>% mean()
  }
  ...
  Cor_at_zero <- 
    cov2cor(tcrossprod(Lambda) + 1 * diag(q))
  ...
  results_gdiff <- table_gdiff(list(perf_spiox_gibbs, perf_spiox_mhfull, perf_spiox_clust, perf_spmeshed, perf_nngp)) %>% 
    as.data.frame() %>% mutate(sim = i)
  results_margs <- table_marginals(list(perf_spiox_gibbs, perf_spiox_mhfull, perf_spiox_clust, perf_spmeshed, perf_nngp)) %>% 
    as.data.frame() %>% mutate(sim = i, variable=1:n())
  colnames(results_gdiff)[1:5] <- colnames(results_margs)[1:5] <- c("spiox_gibbs", "spiox_mhfull", "spiox_clust", "spmeshed", "nngp") 
  
  mat_lt <- function(mat){
    return( mat %>% `[`(lower.tri(.)) )
  }
  
  extract_phi <- function(theta_mcmc){
    return(
      theta_mcmc %>% tail(c(NA, NA, mcmc_keep)) %>% apply(1:2, mean) %>% `[`(1,)
    )
  }
  ...
  results_estphi <- data.frame(spiox_gibbs = spiox_gibbs_out$theta %>% extract_phi(),
                              spiox_mhfull = spiox_mhfull_out$theta %>% extract_phi(),
                              spiox_clust = spiox_clust_out$theta %>% extract_phi(),
                              nngp = nngp_phi,
                              spmeshed = NA,
                              sim = i)
  ...
  save(file=g("simulations/lmc_m/results_{i}.RData"), list=c("results"))
